import csv
from .platform import Master, Node, Cloud, Docker
...
def update_task_queue(master, cur_time, master_id):
    ...
    tmp_list = sorted(tmp_list, key=lambda x: (x[2], x[1])) # (end_time, start_time)
...
def update_docker(node, cur_time, service_coefficient, POD_CPU, POD_MEM):
    done = [0, 0]
    undone = [0, 0]
    done_kind = []
    undone_kind = []
        ...
        for j in range(len(node.service_list)):
            ...
            if node.task_queue[i][0] == node.service_list[j].kind:
                if node.service_list[j].available_time > cur_time:
                    continue
                if node.service_list[j].available_time <= cur_time:
                    to_do_cpu = (node.task_queue[i][3]) / node.service_list[j].cpu
                    # to_do_mem = (node.task_queue[i][4]) / node.service_list[j].mem
                    if cur_time + to_do_cpu <= node.task_queue[i][2] and (node.cpu >= POD_CPU * service_coefficient[node.task_queue[i][0]] and node.mem >= POD_MEM * service_coefficient[node.task_queue[i][0]]):
                        node.cpu = node.cpu - POD_CPU * service_coefficient[node.task_queue[i][0]]
                        node.mem = node.mem - POD_MEM * service_coefficient[node.task_queue[i][0]]
                        node.service_list[j].available_time = cur_time + to_do_cpu
                        node.service_list[j].doing_task = node.task_queue[i]
                        del node.task_queue[i]
                        flag = 1

                    elif cur_time + to_do_cpu > node.task_queue[i][2]:
                        ...
                        pass
                    elif node.mem < POD_MEM * service_coefficient[node.task_queue[i][0]]:
                        pass
